...
import {
  ...
  Account,
  MessageStatus,
} from "../app/layout/chat/models/ChatItem.model";
...
export const fetchMessageCollaboration = (collaborationid: string) => {
  const listMessages: Message[] = lc.getItemLC(
    lc.LCName.Messages + collaborationid
  );
  if (listMessages != null) {
    return Promise.all(listMessages);
  } else {
    return message.fetchMessage(collaborationid).then(async (messages) => {
...
export const clearUnreadMessages = (collaborationId: string) => {
  return message.updateUnreadMessages(collaborationId);
};
