    ...
    if (
      ...
      if(req.entry[0].changes[0].value.metadata){
        if(req.entry[0].changes[0].value.metadata.display_phone_number){
          display_phone_number = req.entry[0].changes[0].value.metadata.display_phone_number
        }
      }
      ...
      if(req.entry[0].changes[0].value.contacts[0]){
        if(req.entry[0].changes[0].value.contacts[0].profile){
          if(req.entry[0].changes[0].value.contacts[0].profile.name){
            profile_name = req.entry[0].changes[0].value.contacts[0].profile.name;
          }
        }
      ...
      if(req.entry[0].changes[0].value.messages[0]){
        ...
        if(req.entry[0].changes[0].value.messages[0].timestamp){
          messages_timestamp = req.entry[0].changes[0].value.messages[0].timestamp;
        }
        ...
        if(messages_type == 'text'){
          if(req.entry[0].changes[0].value.messages[0].text){
            if(req.entry[0].changes[0].value.messages[0].text.body){
              messages_text = req.entry[0].changes[0].value.messages[0].text.body;
            }
          }
        }
      }
      ...
      if(!customer[0]){
        const customerRef = await db.collection('customers')
        customerRef.add({
          ...
          customerRef = createRef('customer', ref)
        }).catch(error => {
          //TODO error harus ngapain
        });
      }else{
        customerRef = createRef('customer', customer[0].id)
      }
      
      // get company by (to) phone number
      const company = await getAccountByPhone(display_phone_number);
      // if company is exist, createRef from companyID
      if(company[0]){
       companyRef = createRef('company', company[0].company.id)
      }
      
      // get collaboration, yang di cek adalah jika customer sama, company sama 
      let getCollaboration = await getCollaborationByCustomerAndCompany(customerRef, companyRef);
      console.log("getCollaboration : " + getCollaboration)
      let collaboration;
      // cek jika percakapan pertama maka buat collaboration
      if(!getCollaboration[0]){
        await collaborationRef.add({
          created: new Date(),
          updatedAt: new Date(),
      ...
      let collaborationMessagesRef = await db.collection('collaborations/'+collaboration.id+'/messages');

      // create message
      await collaborationMessagesRef.add({
        channel: "whatsapp",
        createdAt: new Date(),
        updatedAt: new Date(),
